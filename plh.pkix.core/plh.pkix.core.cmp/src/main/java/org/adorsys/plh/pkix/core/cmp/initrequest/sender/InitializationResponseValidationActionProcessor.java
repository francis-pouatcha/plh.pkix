...
public class InitializationResponseValidationActionProcessor implements ActionProcessor{
	...
	public void process(ActionContext actionContext) {
		...
		checker.checkNull(responseMessage,requests, userAccount);
		...
		try {
			...
			PKISignedMessageValidator signedMessageValidator = new PkiMessageChecker().check(responseMessage,userAccount.getTrustedContactManager());
			...
			ASN1CertValidationResults asn1CertValidationResults = new ASN1CertValidationResults(
					certValidationResults.toArray(new ASN1CertValidationResult[certValidationResults.size()]));

			ASN1Action nextAction = new ASN1Action(
					cmpRequest.getTransactionID(), 
					new DERGeneralizedTime(new Date()), 
					UUIDUtils.newUUIDasASN1OctetString(), 
					new DERIA5String(InitializationResponseValidationPostAction.class.getName()));
