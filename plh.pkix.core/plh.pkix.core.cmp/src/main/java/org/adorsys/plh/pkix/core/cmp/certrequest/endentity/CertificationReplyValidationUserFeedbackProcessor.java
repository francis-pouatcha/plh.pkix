...
public class CertificationReplyValidationUserFeedbackProcessor implements ActionProcessor{
	private final BuilderChecker checker = new BuilderChecker(CertificationReplyValidationUserFeedbackProcessor.class);
	...
	public void process(ActionContext actionContext) {
		...
		ASN1Action nextAction = new ASN1Action(
			...
			new DERIA5String(CertificationReplyValidationUserFeedbackPostAction.class.getName()));
		...
		requests.setResultAndNextAction(cmpRequest, null, new DERIA5String(ProcessingStatus.SUCCESS), nextAction, certValidationResults);
