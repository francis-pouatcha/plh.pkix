...
public class CertReqResponseActionProcessor implements ActionProcessor {
	...
	private final BuilderChecker checker = new BuilderChecker(CertReqResponseActionProcessor.class);
	...
	public void process(ActionContext context) {
		...
		try {
			...
			ASN1CertificationResults asn1CertificationResults = ASN1CertificationResults.getInstance(actionData);			
			...
			ASN1CertificationResult[] certificationResults = asn1CertificationResults.toResultArray();
			List<CertResponse> certResponses = new ArrayList<CertResponse>();
			// Use set to filter duplicates. 
			// ToDo: check that the equals method of org.bouncycastle.asn1.x509.Certificate works.
			...
			for (int i = 0; i < certReqMsgArray.length; i++) {
				...
				if(asn1CertificationResult==null) continue;
				
				ASN1CertificateChain certificateChain = asn1CertificationResult.getCertificateChain();
				org.bouncycastle.asn1.x509.Certificate[] certArray = certificateChain.toCertArray();
				CertResponse certResponse = createCertResponse(certArray[0], certReq.getCertReqId());
			...
			CMPCertificate[] caPubs = caPublicKeyList.toArray(new CMPCertificate[caPublicKeyList.size()]);
			...
			X500Name senderDN = X500NameHelper.readSubjectDN(myCertificateHolder);			
			...
			ProtectedPKIMessageBuilder protectedPKIMessageBuilder = 
					new ProtectedPKIMessageBuilder(new GeneralName(senderDN), certificateRecipient)
		...
		if(executeAction)
			GenericCertResponseActionRegistry.executeAction(cmpRequest, context);// execute

	}

	private CertResponse createCertResponse(org.bouncycastle.asn1.x509.Certificate bcCertificate, ASN1Integer certReqId){
