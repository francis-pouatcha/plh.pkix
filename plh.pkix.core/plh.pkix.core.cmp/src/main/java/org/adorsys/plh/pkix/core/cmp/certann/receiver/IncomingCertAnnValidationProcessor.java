...
public class IncomingCertAnnValidationProcessor implements ActionProcessor {

	private final BuilderChecker checker = new BuilderChecker(IncomingCertAnnValidationProcessor.class);
	public void process(ActionContext actionContext) {
		...
		cmpRequest = new CMPRequest(transactionID, new DERGeneralizedTime(new Date()), 
				new ASN1Integer(PKIBody.TYPE_CERT_ANN), new DERUTF8String(KeyIdUtils.hexEncode(transactionID)));
		...
		try {
			...
			ASN1Action nextAction = new ASN1Action(
				...
				new DERIA5String(IncomingCertAnnValidationPostAction.class.getName()));
...
//			ErrorMessageHelper.processError(cmpRequest, requests, errorMessage);
		...
		if(executeAction)
			GenericIncomingCertAnnActionRegistry.executeAction(cmpRequest, actionContext);// execute
	}
}
