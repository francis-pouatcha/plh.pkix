...
public class ContactManagerImpl implements ContactManager {
	...
	public ContactManagerImpl(KeyStoreWraper mainKeyStoreWraper, FileWrapper contactsDir) {
		...
		// scan if logged in
		if(mainKeyStoreWraper.isAuthenticated())rescan();
	}
	...
	public <T extends Entry> T findMessageEntryByEmail(Class<T> klass,
		...
		for (String email : emails) {
			...
			String keyStoreId = contactIndex.findKeyStoreIdByEmail(email.toLowerCase());
	 ...
	 */
	@Override
	public <T extends Entry> List<T> findMessageEntriesByEmail(
	...
	@Override
	public <T extends Entry> List<T> findCaEntriesByEmail(Class<T> klass,
			String... emails) {
		if(emails==null || emails.length==0) return null;
		Set<String> visitedIds = new HashSet<String>();
		List<T> result = new ArrayList<T>();
		for (String email : emails) {
			...
			KeyStoreWraper loadedKeyStore = loadKeyStore(keyStoreId);
			if(loadedKeyStore==null) continue;
			List<T> entries = loadedKeyStore.findCaEntriesByEmail(klass, email);
			if(entries!=null) result.addAll(entries);
		}
		return result ;
	}
	...
	public <T extends Entry> T findCaEntryByEmail(Class<T> klass,
		...
		for (String email : emails) {
			...
			KeyStoreWraper loadedKeyStore = loadKeyStore(keyStoreId);
			if(loadedKeyStore==null) continue;
			T caEntry = loadedKeyStore.findCaEntryByEmail(klass, email);
			if(caEntry!=null) return caEntry;
		}
		return null;
	}
	...
	@Override
	public boolean login(char[] accountPass) {
		mainKeyStoreWraper.setKeyPass(accountPass);
		boolean authenticated = isAuthenticated();
		if(authenticated) rescan();
