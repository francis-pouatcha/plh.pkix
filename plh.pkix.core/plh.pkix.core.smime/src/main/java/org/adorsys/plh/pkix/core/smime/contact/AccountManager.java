...
public class AccountManager {
	...
	public AccountManager(ActionContext accountContext, FileWrapper accountDir, char[] accountPass){		
		this.accountDir = accountDir;
		this.accountContext = accountContext;
		FileWrapper contactsDir = accountDir.newChild(CONTACTS_DIR_NAME);
		KeyStoreWraper accountStore = accountDir.getKeyStoreWraper();
		accountStore.setKeyPass(accountPass);
		contactManager = new ContactManagerImpl(accountStore, contactsDir);
		accountContext.put(ContactManager.class, contactManager);
	}
