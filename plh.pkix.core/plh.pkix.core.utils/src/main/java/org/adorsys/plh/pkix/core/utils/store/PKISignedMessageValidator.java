		...
		for (X509CertificateHolder x509CertificateHolder : signerCertificates) {
			...
			if (cert != null) {
				...
				try {
					...
					ValidationResult validationResult = new ValidationResult(review,true, errors, notifications,certPathAndOrigin.getUserProvidedFlags());
					validationResult.setReturnValue(x509CertificateHolder);
					results.add(validationResult);
				} catch (CertPathReviewerException cpre) {
					// cannot initialize certpathreviewer - wrong parameters
					errors.add(cpre.getErrorMessage());
					ValidationResult validationResult = new ValidationResult(null,false, errors, notifications, null);
					validationResult.setReturnValue(x509CertificateHolder);
					results.add(validationResult);
				}
			} else
	...
	public ValidationResult getValidationResult(X509CertificateHolder signer)
		...
		if (signerCertificates.contains(signer)) {
			for (ValidationResult validationResult : results) {
				if(signer.equals(validationResult.getReturnValue())) return validationResult;
			}
		}
